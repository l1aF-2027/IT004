...
SELECT MH.TENMH, MH.MAMH, KQT.KQUA, COUNT (DISTINCT KQT.MAHV) AS SOHOCVIEN
FROM MONHOC MH 
	JOIN
(
	SELECT KQT1.MAHV, KQT1.MAMH, KQT1.KQUA
	FROM KETQUATHI KQT1
		LEFT JOIN KETQUATHI KQT2 ON KQT1.MAHV = KQT2.MAHV AND KQT1.MAMH = KQT2.MAMH AND KQT1.LANTHI < KQT2.LANTHI
	WHERE KQT2.MAHV IS NULL
) KQT ON KQT.MAMH = MH.MAMH
...
SELECT NAM, HOCKY, MAGV, COUNT(MAMH) AS SOMH, COUNT(MALOP) AS SOLOP 
FROM GIANGDAY
GROUP BY HOCKY, NAM, MAGV
ORDER BY NAM, HOCKY, MAGV ASC;

--- Câu 29. Trong từng học kỳ của từng năm, tìm giáo viên (mã giáo viên, họ tên) giảng dạy nhiều nhất.
SELECT GD.NAM, GD.HOCKY, GD.MAGV, GV.HOTEN, GD.SOMONDAY
FROM 
(
...
(
	...
	JOIN HOCVIEN HV ON DAT.MAHV = HV.MAHV
...
FROM 
(
	SELECT KQT.MAHV, SLT.SOMH
	FROM KETQUATHI KQT
		JOIN SO_MON_THI SLT ON KQT.MAHV = SLT.MAHV
	WHERE NOT EXISTS 
	(
		SELECT 1 
		FROM KETQUATHI B 
		WHERE KQT.MAHV = B.MAHV AND KQT.MAMH = B.MAMH AND KQT.LANTHI < B.LANTHI
	) AND KQUA = 'Dat'
	GROUP BY KQT.MAHV, SLT.SOMH
	HAVING COUNT(KQUA) = SLT.SOMH
) DAT
	JOIN HOCVIEN HV ON DAT.MAHV = HV.MAHV

--- Câu 33 và câu 34. Em thấy nội dung câu hỏi câu 33 và 34 hơi khó hiểu ạ vì em không hiểu ý nó là tất cả các môn học là tất cả môn học được dạy ở trường hay tất cả
--- các môn học mà học sinh thi (nếu thế thì nó lại giống câu 31,32) nên dưới đây em làm theo kiểu tất cả môn học được dạy ở trường nên kết quả sẽ ra bảng trống ạ.

--- Câu 33. Tìm học viên (mã học viên, họ tên) đã thi tất cả các môn và đều đạt (chỉ xét lần thi thứ 1).
WITH SO_MON AS
(
	SELECT COUNT(MAMH) AS SOMH 
	FROM MONHOC
)
...
FROM 
(
	SELECT KQT.MAHV, SM.SOMH
	FROM KETQUATHI KQT, SO_MON SM
	WHERE LANTHI = 1 AND KQUA = 'Dat'
	GROUP BY KQT.MAHV, SM.SOMH
	HAVING COUNT(KQUA) = SM.SOMH
) DAT
	JOIN HOCVIEN HV ON DAT.MAHV = HV.MAHV
